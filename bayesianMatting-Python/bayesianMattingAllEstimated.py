...
for n in range(1):
  ...
  # Import input images, and transfer them to double datatype
  # ranged from 0 to 1
    ...
    for i in range(rows):
      for j in range(cols):
        if unknown[i, j]:
          ...
          # ensure enough foreground, background pixels and non-large
          # neighborhood with previously estimated pixels, with
          # maximum neighborhood of 750
          ...
          if side_length >= max_side_length:
            ...
            # if num_fg_pixels == num_bg_pixels, set to foreground
            alpha_matte[i, j, :] = 1
            # progress bar
            bar()
            continue
  ...
  plt.axis("off")
  plt.savefig('./results/one_loop/2all_previous/alpha_matte_' +
              str(No_input) + '.png')
  ...
  plt.imshow(F)
  plt.axis("off")
  plt.savefig('./results/one_loop/2all_previous/F_' + str(No_input) + '.png')

  plt.figure()
  plt.title('Output background')
  plt.imshow(B)
  plt.axis("off")
  plt.savefig('./results/one_loop/2all_previous/B_' + str(No_input) + '.png')
...
with open("all_previous2.txt", "w") as file:
