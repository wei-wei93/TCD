...
for n in range(1):
    ...
    for i in range(rows):
      for j in range(cols):
        if unknown[i, j]:
          ...
          # ensure enough foreground, background pixels and
          # non-large neighborhood with previously estimated
          # pixels, with maximum neighborhood of 750
  ...
  plt.axis("off")
  plt.savefig('./results/one_loop/1no_previous/alpha_matte_' +
              str(No_input) + '.png')
  ...
  plt.imshow(F)
  plt.axis("off")
  plt.savefig('./results/one_loop/1no_previous/F_' + str(No_input) + '.png')

  plt.figure()
  plt.title('Output background')
  plt.imshow(B)
  plt.axis("off")
  plt.savefig('./results/one_loop/1no_previous/B_' + str(No_input) + '.png')
...
with open("no_previous2.txt", "w") as file:
