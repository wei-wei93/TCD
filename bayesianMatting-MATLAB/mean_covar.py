...
def calmean(pixel, weight):
    ...
    mean_r = (np.sum(np.dot(pixel_r, weight)))/W
    mean_g = (np.sum(np.dot(pixel_g, weight)))/W
    mean_b = (np.sum(np.dot(pixel_b, weight)))/W
...
def calcovar(pixel, weight, mean_rgb):
    W = np.sum(weight)
    # get the values for each of the three RGB values
    pixel_r = pixel[0, :]
    pixel_g = pixel[1, :]
